...
import dev.gerasimova.service.EntityMapper;
...
public class ProductController {
    ...
    /**
     * Выводит всю информацию о продукте из хранилища по его идентификатору пользователю,
     * включая отзыве о продукте.
     *
     * @param id уникальный идентификатор продукта
     * @return ProductDetailsDto - дто для вывода всей информации о продукте.
     */
    @Operation(summary = "Получить детали о продукт по ID", description = "Находит информацию о продукте по его идентификатору с отзывами")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Продукт найден, детальная информация представлена",
    ...
    public ResponseEntity<ProductDetailsDto> detailsProductById(@PathVariable Long id) {
        ProductResponseDto product = productService.getProductById(id);
        List<ProductReviewDto> reviewList = productReviewService.getReviewsForProduct(id);
        return ResponseEntity.ok(entityMapper.toDto(product, reviewList));
    }
}
