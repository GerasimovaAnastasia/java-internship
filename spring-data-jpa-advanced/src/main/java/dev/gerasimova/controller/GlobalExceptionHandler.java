...
public class GlobalExceptionHandler {
     ...
     *
     * @param e исключение ProductException
     ...
     * Нужен, когда создаются дупликаты объектов (одинаковые название продукта и категория)
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrity(DataIntegrityViolationException e) {
        if (e.getMessage().contains("products_unique")) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("error", "Продукт с таким названием и категорией уже существует"));
        }
